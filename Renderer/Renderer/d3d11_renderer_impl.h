	...
	struct renderer_t::impl_t
	{
		...
		*/
		...
		impl_t(native_handle_type window_handle, view_t& default_view)
		{
			...
			XMVECTOR eyepos = XMVectorSet(0.0f, 15.0f, -25.0f, 1.0f);
		...
		void draw_view(view_t& view)
		{
			...
			//mvp.view = (XMMATRIX&)view.view_mat;
			...
			{
				if (!Models[i]->CanRender)
				{
					continue;
				}
				unsigned int stride = sizeof(TerrainVertex);
				unsigned int offset = 0;
				context->IASetVertexBuffers(0, 1, Models[i]->GetVertexBuffer(), &stride, &offset);
				context->IASetIndexBuffer(Models[i]->GetIndexBuffer(), DXGI_FORMAT_R32_UINT, 0);
				context->VSSetShader(Models[i]->GetVertexShader(), nullptr, 0);
				context->PSSetShader(Models[i]->GetPixelShader(), nullptr, 0);
				context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
				context->VSSetConstantBuffers(0, 1, &constant_buffer[CONSTANT_BUFFER::MVP]);
				context->IASetInputLayout(Models[i]->GetInputLayout());
				context->UpdateSubresource(constant_buffer[CONSTANT_BUFFER::MVP], 0, nullptr, &mvp, 0, 0);
				//context->UpdateSubresource(*Models[i]->GetVertexBuffer(), 0, nullptr, Models[i]->Vertices.data(), 0, 0);

				context->DrawIndexed(Models[i]->GetVertexCount(), 0, 0);
			}
		...
		void SetImGuiFuncPointer(void (*renderImGui)(void))
		{
