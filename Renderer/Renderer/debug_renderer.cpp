...
namespace
{
	...
	constexpr size_t MAX_LINE_VERTS = 200000; 
...
namespace end
{
	namespace debug_renderer
	{
		...
		void add_line(float3 point_a, float3 point_b, float4 color_a, float4 color_b)
		{
			...
			if (line_vert_count + 2 > MAX_LINE_VERTS) { return; }

			line_verts[line_vert_count++] = end::colored_vertex(point_a, color_a);
			line_verts[line_vert_count++] = end::colored_vertex(point_b, color_b);
		}
